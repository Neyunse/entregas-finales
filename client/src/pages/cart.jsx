...
const Cart = () => {
      ...
      const GetCartData = async (id) => {
            await axios(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/cart/my/${id}`, {
                  headers: {
      ...
      const removeCallback = async (id) => {
            ...
            if (localStorage.getItem("user") != undefined) {
                  await axios(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/cart/del/products/${id}`, {
                        method: "DELETE",
                        headers: {
                              "Content-Type": "application/json",
                              "Authorization": " Bearer " + JSON.parse(localStorage.getItem("user")).access_token
      ...
      const checkout = async () => {
            ...
            const options = {
                  method: "POST",
                  url: `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/checkout`,
                  headers: {
                        "Content-Type": "application/json",
                        "Authorization": 'Bearer ' + JSON.parse(localStorage.getItem("user")).access_token
                  }
